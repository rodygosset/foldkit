...
export function makeInterruptibleQueryModel<A, AI, E, EI>(
  data: Schema.Codec<A, AI>,
  error: Schema.Codec<E, EI>,
) {
  const states = AsyncData.Schema(data, error)
...
> {
  ...
  readonly Fetch: Interrupt extends true
    ? Interruptible.DefinitionWithArgs<
        ...
        Effect.Effect<
          SettledFetchOf<QueryMessage<A, AI, E, EI, true>>,
          never,
          R
        >
      >
    : Command.CommandDefinitionWithArgs<
        ...
        { requestId: typeof Schema.Number },
        Effect.Effect<
          SettledFetchOf<QueryMessage<A, AI, E, EI, false>>,
          never,
          R
        >
      >
...
export function definePlainQuery<Name extends string, A, AI, E, EI, R>(
  ...
  type Message = QueryMessage<A, AI, E, EI, false>['Type']
  ...
  type Model = QueryModel<A, AI, E, EI, false>['Type']
  type Args = undefined
  ...
  const store: CacheStore<Model, Args, A, E, Message, R> = {
    ...
    load: (_args, requestId, _model) => Fetch({ requestId }),
  }

  function forgetSlot(model: Model): UpdateReturn {
    if (AsyncData.isIdle(model.data)) return { model }
  ...
  return {
    ...
    watchSubscription,
    run,
  } satisfies Query<Name, A, AI, E, EI, R, false>
}

export function defineInterruptibleQuery<Name extends string, A, AI, E, EI, R>(
  config: QueryConfig<Name, A, AI, E, EI, R>,
): Query<Name, A, AI, E, EI, R, true> {
  const ModelSchema = makeInterruptibleQueryModel(config.data, config.error)
  const Message = makeInterruptibleQueryMessage(config.data, config.error)
  type Message = QueryMessage<A, AI, E, EI, true>['Type']
  const Fetch = Command.define(`Fetch${config.name}`, {
    args: {
      requestId: Schema.Number,
      instanceId: Schema.String,
  ...
  type Model = QueryModel<A, AI, E, EI, true>['Type']
  type Args = undefined
  type UpdateReturn = Update.Return<Model, Message, R>
  ...
  const store: InterruptibleCacheStore<Model, Args, A, E, Message, R> = {
    ...
    isCurrent: (model, _args, requestId) =>
      sameRequest(model.maybePendingRequestId, requestId),
    load: (_args, requestId, model) =>
      Fetch({ requestId, instanceId: model.instanceId }),
    interrupt: (model, _args, intent) =>
      Option.match(model.maybePendingRequestId, {
        onNone: () =>
          Fetch.Interrupt({ instanceId: model.instanceId }, outcome =>
  ...
  function forgetSlot(model: Model): UpdateReturn {
    ...
    if (AsyncData.isPending(model.data))
      return {
        model: forgotten,
  ...
  const update = (model: Model, message: Message): UpdateReturn =>
    Message.match<UpdateReturn>(message, {
      ...
      RequestedForget: () => forgetSlot(model),
      SettledFetch({ requestId, result }) {
        if (!store.isCurrent(model, undefined, requestId)) return { model }
      ...
      CompletedCancelFetch({ requestId, outcome, intent }) {
        if (!store.isCurrent(model, undefined, requestId)) return { model }
  ...
  return {
    ...
    watchSubscription,
    run,
  } satisfies Query<Name, A, AI, E, EI, R, true>
}
...
export function defineQuery<Name extends string, A, AI, E, EI, R>(
  config: QueryConfig<Name, A, AI, E, EI, R> & { readonly interrupt?: boolean },
): Query<Name, A, AI, E, EI, R, true> | Query<Name, A, AI, E, EI, R, false> {
  if (config.interrupt === true) {
    return defineInterruptibleQuery(config)
  }
