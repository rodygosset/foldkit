...
export const TABS_ID = 'api-cache-http-api-tabs'
    ...
    BlogClient.of(
      /* eslint-disable-next-line @typescript-eslint/consistent-type-assertions */
      {
        blog: {
          ...
          getPost: (request: {
            readonly params: { readonly postId: string }
          }) =>
            Effect.gen(function* () {
...
export const postsQuery = BlogClient.query('Posts', 'blog', 'listPosts')

export const statsQuery = BlogClient.query('Stats', 'blog', 'getStats')

export const postDetailQuery = BlogClient.query('PostDetail', 'blog', 'getPost')
...
const foldTabsOutMessage = Tabs.OutMessage.match<
  Update.Step<Model, Message, BlogClient>,
  Tabs.OutMessage<Tab>
>({
  Selected:
    ({ value }) =>
...
export const update = (model: Model, message: Message) =>
  Message.match<UpdateReturn>(message, {
    ...
    ClickedRetryPostDetail: ({ postId }) =>
      postDetailChild.revalidateOrLoad(model, { params: { postId } }),
...
export const subscriptions = Subscription.make<Model, Message, BlogClient>()(
  entry => ({
  ...
  h.header(
    ...
    [
      ...
      h.p(
        ...
        [
          'Query.HttpApi.Service.query builds the same Query and KeyedQuery Submodels from an Effect HttpApi endpoint.',
        ],
      ),
    ],
  )
...
const isPostDetailCached = (
  ...
  AsyncData.hasData(
    postDetailQuery.read(postDetailById, { params: { postId } }),
  )
...
const postDetailView = (
  ...
  const postDetailData = postDetailQuery.read(model.postDetailById, {
    params: { postId },
  })
...
type QueryError = string | Query.HttpApi.HttpApiClientError

const formatQueryError = (error: QueryError): string => {
